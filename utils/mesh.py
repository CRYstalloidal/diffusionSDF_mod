...
def create_mesh(
    model, shape_feature, filename, N=256, max_batch=1000000, level_set=0.0, occupancy=False,if_from_latent=False, device = "cuda"
):
    ...
    while head < cube_points:
        ...
        if if_from_latent:
            pred_sdf = model.gen_from_latent(shape_feature.to(device), query.to(device))
        else:
            pred_sdf = model(shape_feature.to(device), query.to(device))
